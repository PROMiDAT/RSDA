...
gplot <- function(data, color=NA, matrix.form=NA, border=FALSE, show.type = FALSE, reduce =FALSE, fill=NA ,background.color = "white"){
  ...
  size.text <- 1 - 0.02*sum(matrix.form)
  size.text <- ifelse(size.text < 0.35,0.35,size.text)
  ...
  pushViewport(viewport(layout = grid.layout(nrow(layout), ncol(layout), heights = unit(1/max(matrix.form),"native"), widths = unit(1/max(matrix.form),"native"))))
  # else
  ...
  # se obtiene por fila o por columna segun byColm
    for(index.col in 1:data$M){
      ...
      switch (var.data$sym.var.types,
              "$I" = view.gplot( sym.interval.gplot(var.data, color, border, show.type,fill, size.text, background.color), pos$row, pos$col),
              "$C" = view.gplot( sym.continuos.gplot(var.data, color, border, show.type, size.text, background.color), pos$row,pos$col),
              "$H" = view.gplot( sym.hist.gplot(var.data, color, border, show.type,fill, background.color), pos$row,pos$col),
              "$M" = view.gplot( sym.modal.gplot(var.data, color, border, show.type, reduce, fill, background.color), pos$row,pos$col),
              "$S" = view.gplot( sym.set.gplot(var.data, color, border, show.type, reduce, fill, background.color), pos$row,pos$col)
      )
    }
  }
}
