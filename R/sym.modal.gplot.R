#' sym.modal.gplot
#' @keywords internal
sym.modal.gplot <- function(info, color=c("black"), border=FALSE, show.type = TRUE, reduce=FALSE, fill = "white", background.color = "gray"){
  ...
  ggplot(data = data.frame(x =  names,y=as.numeric(mt)), aes(x,y)) +
    ...
    ggtitle(paste(info$sym.var.names,ifelse(show.type," (Modal)",""))) + #Titulo del plot
