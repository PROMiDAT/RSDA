#' sym.modal.gplot
#' @keywords internal
sym.modal.gplot <- function(info, color=c("black"), border=FALSE, show.type = TRUE, reduce=FALSE, fill = "white",font.size = 1, background.color = "gray"){
  if(info$sym.var.types != "$M") #Si el tipo no es el correcto
    stop("The data type is wrong, only $M are accepted")

  mt <- info$data #Los datos de la tabla simbolica (solo modal)
  ...
  ggplot(data = data.frame(x =  names,y=as.numeric(mt)), aes(x,y))  +
    geom_bar(stat="identity", color=color, fill=fill) + #Crea las barras
    ggtitle(paste(toupper(info$sym.var.names),ifelse(show.type," (Modal)",""))) + #Titulo del plot
    scale_y_continuous(limits = c(0,1), labels = scales::percent)+ #Limites del eje y
