#' sym.hist.gplot
#' @keywords internal
sym.hist.gplot <- function(info,color=c("black"), border=FALSE, show.type = TRUE, fill = "white",font.size = 1, background.color = "gray"){
  ...
  ggplot(data.frame(dt = dataTemp), aes(x=dt)) +
    ...
    scale_y_continuous(limits = c(0,100), labels = function(x)paste0(x, "%")) + #Limites del eje y
    ggtitle(paste(toupper(info$sym.var.names),ifelse(show.type," (Histogram)",""))) + #Titulo del plot
