...
sym.continuos.gplot <- function(info, color=c("black"), border=FALSE, show.type = TRUE, font.size = 1, background.color = "gray"){
  ...
  ggplot() +
    ...
    geom_text(aes(continuos,2,label = continuos), size=10*font.size) + # El numero continuo como leyenda
    ggtitle(paste(info$sym.var.names,ifelse(show.type," (Continuos)",""))) + # Titulo del plot
