...
process.histogram.variable <- function(variableName, concept, dataTable) {
  concept. <- as.name(stringr::str_replace_all(concept, "[\\[\\]]", ""))
  ...
  data. <- dataTable %>% dplyr::group_by(!!concept.) %>% dplyr::do(!!cal.prop)

  data. <- data. %>% tidyr::unite("interval",to, from, sep = ",") %>%
    dplyr::mutate(interval = c(paste0("[",head(interval, n() - 1), ")"), interval[n()])) %>%
